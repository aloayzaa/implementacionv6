...
use App\Http\Requests\CashMovementStore;
...
class CashMovementController extends Controller
{
    ...
    {
        $banks = BankDocument::lista_banco('MOV_CAJA');
    ...
    public function index(Request $request)
    {
        ...
        $data['view'] = link_view('Tesoreria', 'Transacción', 'Moviminetos de Caja', '');
        $data['header'] = headeroptions($this->var, 'list', '', '', $this->privilegios(), $this->user);
        $data['navbar_menu'] = navbar_menu($this->privilegios_navbar_menu(), $this->user);
        return view('cashmovement.list', $data);
    }
    ...
    public function create()
    {
        ...
        $data['route'] = route('store.cashmovement');
        $data['proceso'] = 'create';
        $data['var'] = $this->var;
        $data['mediopagos'] = PaymentType::all()->where('estado',1);
        $data['cobradores'] = Seller::where('estado', 1)->get();
        $data['bancos'] = Bank::where('efectivo', 1)->where("estado",1)->get();
        ...
        $data['view'] = link_view('Tesoreria', 'Transacción', 'Movimientos de Caja', '');
        ...
        return view('cashmovement.create', $data);
    }
    ...
    private function llenar_referencias($data, $tercero_filtro, $tcambio)
    {
        ...
        foreach ($data as $d) {
            $cuenta = AccountingPlan::findOrFail($d->cuenta_id);
    ...
    public function store(CashMovementStore $request)
    {
        ...
        try {
            ...
            $banco->cobrador_id = (isset($request->cobrador)) ? $request->cobrador : null;
            ...
            $id = DB::transaction(function () use ($banco, $request) {
                ...
                $this->setestadoref($banco->estado);
    ...
    public function edit(Request $request, $id)
    {
        ...
        $data['proceso'] = 'edita';
        $data['var'] = $this->var;
        $data['bancos'] = Bank::where('efectivo', 1)->get();
        ...
        return view('cashmovement.edit', $data);
    }
    ...
    public function update(CashMovementStore $request, $id)
    {
    ...
    public function agregar_item(CashMovementDetail $request)
    {
        ...
        $retencion = $retencion_renta = $retencion_pension = 0; //carrito

        $data = json_decode($request->data);
        $count = count($data);

        $percepcion = (isset($request->txt_percepcion)) ? $request->txt_percepcion : 0;
        ...
        if ($count > 0) {
            ...
            if ($request->rowId != ''){
                $this->actualizar_detalle($request->rowId, $operacion, $tercero, $request->amount, $pcg, $request->comment, $request->txt_fecha, $cci, $centrocosto, $proyecto, $actividad, $percepcion);
            }else {
                ...
                Cart::instance($this->instancia)->add(['id' => $item + 1, 'name' => 'un nombre', 'qty' => 1, 'price' => 0.00,
                    'options' => [
                        ...
                        'retencion_pension' => 0,
                        'cci' => $cci,
                        'total' => $request->amount + $percepcion,
    ...
    public function tipooperacion(Request $request)
    {
        $operation = OperationType::where('tipo', $request->type)->get();
        return $operation;
    }
    ...
    private function actualizar_detalle($rowId, $operacion, $tercero, $importe, $pcg, $glosa, $fecha_proceso, $cci, $cost, $proyecto, $actividad, $percepcion){
    ...
    public function setestadoref($docbanco_estado){
        $estado = 'AT.TOTAL';
        if ($docbanco_estado == 'ANULADO'){ $estado = 'APROBADO'; }
        foreach (Cart::instance($this->instancia)->content() as $cart) {
            if ($cart->options->esanticipo == 1 && $cart->options->referencia_id != null){
                DocumentToPay::where('id', $cart->options->referencia_id)->update(['estado' => $estado ]);
            }
        }
    }
