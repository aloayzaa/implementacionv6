...
class GenerarXMLNDebito21
{
     ...
     * Handle the event.
     *
     * @param  GenerarXMLNDebito21Events  $event
     * @return void
     */
    public function handle(GenerarXMLNDebito21Events $event)
    {
        ...
        $lcTipoNotaCreDe       = trim($documento->tiponotadb); // TIPO DE NOTA DE CRÉDITO QUE AFECTA EL DOCUMENTO ELECTRÓNICO VER. CATÁLOGO No 9 SUNAT http://www.sunat.gob.pe/legislacion/superin/2014/anexo8-300-2014.pdf 
        $lcDescriSustento      = trim($documento->tiponotadb_dsc); // SUSTENTO DEL TIPO DE ANULACIÓN DE NOTA DE CRÉDIDO O DÉBITO VER. CATÁLOGO No 9 SUNAT http://www.sunat.gob.pe/legislacion/superin/2014/anexo8-300-2014.pdf
        ...
        if ( $documento->totaldscto != 0 ) {
            $lcTotalDescuento  = trim(formatear_numero(abs($documento->totaldscto), 2)); //OPCIONAL - DESCUENTOS TOTALES DEL DOCUMENTO
        } 
        ...
        foreach($documento_detalles as $detalle){
            ...
            if ( ($d_cantidad > 0 && $d_precio > 0) || $d_referencia_id > 0 ){
        ...
        $lcStrXML .= '<DebitNote xmlns="urn:oasis:names:specification:ubl:schema:xsd:DebitNote-2"' . $lcChrEnt;
        ...
        if ($this->cpetiposerv == 1) {// OSE

            if ( !empty($lcDocumentoAfecta) ){  
        ...
        $lcStrXML .= '<cac:RequestedMonetaryTotal>' . $lcChrEnt;
        ...
        if ( is_string($laItems[1][1]) ){
            ...
            for ($lnItem = 1; $lnItem <= count($laItems); $lnItem++){
                ...
                $lcStrXML .= '<cac:DebitNoteLine>' . $lcChrEnt;
                $lcStrXML .= '	<cbc:ID>' . trim($lnItem) . '</cbc:ID>' . $lcChrEnt;
                $respuesta_condicional = ( is_string($laItems[$lnItem][10]) ) ? $laItems[$lnItem][10] : 'NIU';  
                $lcStrXML .= '	<cbc:DebitedQuantity unitCode="' . $respuesta_condicional . '" unitCodeListID="UN/ECE rec 20" unitCodeListAgencyName="United Nations Economic Commission for Europe">' . $laItems[$lnItem][1] . '</cbc:DebitedQuantity>' . $lcChrEnt; 
                $respuesta_condicional = ( in_array($lcTipoIGV, array("11","12","13","14","15","16")) ) ?  "0.00" : $laItems[$lnItem][2];
                $lcStrXML .= '	<cbc:LineExtensionAmount currencyID="' . $lcMoneda . '">' . $respuesta_condicional . '</cbc:LineExtensionAmount>' . $lcChrEnt; 
                ...
                if ( $documento->esgratuito == 1 ) {
                    ...
                    $lcStrXML .= '			<cbc:TaxAmount currencyID="' . $lcMoneda . '">' . $laItems[$lnItem][4] . '</cbc:TaxAmount>' . $lcChrEnt;
                    $lcStrXML .= '			<cac:TaxCategory>' . $lcChrEnt;
                    $respuesta_condicional = ( $laItems[$lnItem][4] == 0) ? "0.00" : $laItems[$lnItem][13];
                    $lcStrXML .= '				<cbc:Percent>' . $respuesta_condicional . '</cbc:Percent>' . $lcChrEnt;
                ...
                if ( is_string($lcTotalICBPER) ) {
                    if ( !empty($lcTotalICBPER) ) {
                        if ( is_string($laItems[$lnItem][8]) ) {
                            $lcStrXML .= '		<cac:TaxSubtotal>' . $lcChrEnt;
                            $lcStrXML .= '			<cbc:TaxAmount currencyID="' . $lcMoneda . '">' . $laItems[$lnItem][8] . '</cbc:TaxAmount>' . $lcChrEnt;
                            $lcStrXML .= '			<cbc:BaseUnitMeasure unitCode="' . $laItems[$lnItem][10] . '">' . trim((int)$laItems[$lnItem][1]) . '</cbc:BaseUnitMeasure>' . $lcChrEnt;		
                            $lcStrXML .= '			<cac:TaxCategory>' . $lcChrEnt;
                ...
                $respuesta_condicional = ( in_array($lcTipoIGV, array("11", "12", "13", "14", "15", "16", "21")) ) ? "0.00" : $laItems[$lnItem][7];
                $lcStrXML .= '		<cbc:PriceAmount currencyID="' . $lcMoneda . '">' . $respuesta_condicional . '</cbc:PriceAmount>' . $lcChrEnt;
                $lcStrXML .= '	</cac:Price>' . $lcChrEnt;
                $lcStrXML .= '</cac:DebitNoteLine>' . $lcChrEnt;
            } // FOR ITEM
        }  // ES STRING EL CÓDIGO
